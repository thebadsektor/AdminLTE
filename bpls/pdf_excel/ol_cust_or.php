...
// order no
$test = me_decrypt($_GET['target']);
//permit_id

$q8545 = mysqli_query($conn,"SELECT * from  geo_bpls_paid_online where order_no = '$test' ");
$r8545 = mysqli_fetch_assoc($q8545);

$id = md5($r8545["permit_id"]);
$or_nos = $r8545["or_no"];

// get payment_id using orn_no
$q811 = mysqli_query($conn, "SELECT * from  geo_bpls_payment where or_no = '$or_nos' ");
$r811 = mysqli_fetch_assoc($q811);
$test = md5($r811["payment_id"]);

$q = mysqli_query($conn, "SELECT * FROM `geo_bpls_payment` WHERE md5(payment_id) = '$test'  ") or die("database error:" . mysqli_error($conn));
...
if ($payment_mode == "CHEC") {
    $qqq = mysqli_query($conn,"SELECT * FROM `geo_bpls_payment_check`  WHERE md5(payment_id) = '$test' ");
...
$total = 0;
// change later
$result = mysqli_query($conn, "SELECT natureOfCollection_tbl.name as sub_account_title, geo_bpls_payment_detail.payment_amount FROM `geo_bpls_payment_detail`  inner JOIN natureOfCollection_tbl on natureOfCollection_tbl.id = geo_bpls_payment_detail.sub_account_no WHERE md5(payment_id) = '$test'  ") or die("database error:" . mysqli_error($conn));
...
$write->printRow();
$write->easyCell('', 'colspan:4; align:L; font-size:10; border:;');
$write->easyCell('Cashier Officer', 'colspan:6; align:C; font-size:8; border:;');
$write->printRow();
