...
$business_name = mysqli_real_escape_string($conn,$r11["business_name"]);
$payment_mode = $r11["payment_frequency_code"];
$q001 = mysqli_query($conn,"SELECT * FROM `geo_bpls_payment_frequency` where payment_frequency_code= '$payment_mode'");
$r001 = mysqli_fetch_assoc($q001);
$payment_mode = $r001["payment_frequency_desc"];
    // Audit Trail =====================================================================================
    // Audit Trail =====================================================================================
        $au_username = $_SESSION["uname"];
        $au_action = "Generate Business Permit";
        $au_desc = "Generate ".$business_name." business permit ";
...
if ($rwww["status_code"] != "RET") {
    ...
    $q2222 = mysqli_query($conn, "SELECT nature_desc, geo_bpls_business_permit_nature.nature_id FROM `geo_bpls_business_permit_nature` inner join geo_bpls_nature on geo_bpls_nature.nature_id = geo_bpls_business_permit_nature.nature_id where md5(permit_id) = '$uniq_id' ");
...
// $pdf->Image('../../bpls/images_file/vf_mbp.jpg', 0, 0, 295.96, 210.5);
        ...
        if ($r11["permit_no"] == null) {
            ...
            if ($nano_scale_code != $r11["scale_code"]) {
                ...
                inner join geo_bpls_business on geo_bpls_business.business_id = geo_bpls_business_permit.business_id
                where permit_no != '' and scale_code != $nano_scale_code and permit_for_year = '$year'  ORDER BY SUBSTRING(permit_no,16,20) DESC LIMIT 1");
                ...
                $q = mysqli_query($conn, "SELECT SUBSTRING(permit_no,16,20) ,  permit_no FROM `geo_bpls_business_permit`
            inner join geo_bpls_business on geo_bpls_business.business_id = geo_bpls_business_permit.business_id
            where permit_no != '' and scale_code = $nano_scale_code and permit_for_year = '$year'  ORDER BY SUBSTRING(permit_no,18,20) DESC LIMIT 1");
                $r = mysqli_fetch_assoc($q);
        ...
        $write->easyCell(str_replace("&AMP;", "&", str_replace("&amp;", "&", strtoupper(utf8_decode($r11["business_name"])))), 'colspan:100; paddingY:0; font-style:BU; align:C; font-size:17;');
        $write->printRow();
...
//  $nature
        ...
        $q22 = mysqli_query($conn,"SELECT * FROM `geo_bpls_signatory` where target_file = 'business_permit'");
        ...
        $esig_status = $r22["esig_status"];

        $write->easyCell('', 'colspan:52; paddingY:0;');
        $write->easyCell(utf8_decode($signatory_name), 'colspan:35; paddingY:0; align:C; font-style:B; font-size:12;');
        $write->easyCell('', 'colspan:18; paddingY:0;');
        $write->printRow();

        if($esig_status == "1"){
            if($e_signatory != ""){
                $pdf->Image('../../bpls/images_file/e_sig/'.$e_signatory, 180, 130, 42.96, 22.5);
            }
        }
        $write->easyCell('', 'colspan:57; paddingY:0;');
        $write->easyCell($signatory_position, 'colspan:25; paddingY:0; align:C; font-style:I; font-size:12;');
        $write->easyCell('', 'colspan:18; paddingY:0;');
        $write->printRow();
        ...
        // check kung anong payment mode at ilan qtr ang binayaran
                    $q848d = mysqli_query($conn,"SELECT payment_frequency_code FROM `geo_bpls_business_permit` where md5(permit_id) = '$uniq_id' ");
                    $r848d = mysqli_fetch_assoc($q848d);
                    $payment_frequency_code = $r848d["payment_frequency_code"];

                    $q348d = mysqli_query($conn,"SELECT payment_part FROM `geo_bpls_payment` where md5(permit_id) = '$uniq_id' ");
                    $payment_part = 0;
        ...
        $write->easyCell('', 'colspan:15; paddingY:0;');
        $write->easyCell('Series :', 'colspan:8; paddingY:0; font-size:10;');
        ...
        $write->easyCell('', 'colspan:15; paddingY:0;');
        $write->easyCell('Payment Mode :', 'colspan:9; paddingY:0; font-size:10;');
        $write->easyCell($payment_mode, 'colspan:76; paddingY:0; font-style:B; font-size:10;');
        $write->printRow();
...
// $pdf->AddFont('oldenglishn');
    ...
    $write->easyCell('' . utf8_decode($r["b_add"]). '', 'colspan:45; paddingY:0; border:B;');
    $write->easyCell('', 'colspan:13; paddingY:0;');
    $write->printRow();
...
// $date111 = date('l jS \of F Y', $aa);
    ...
    $q22 = mysqli_query($conn,"SELECT * FROM `geo_bpls_signatory` where target_file = 'business_retirement_tres'");
    $r22 = mysqli_fetch_assoc($q22);
    $signatory_name = $r22["signatory_name"];
    $signatory_position = $r22["signatory_position"];

    $write->easyCell('', 'colspan:48; paddingY:0;');
    $write->easyCell($signatory_name, 'colspan:34; paddingY:0; align:C; font-size:12; font-style:B;');
    $write->easyCell('', 'colspan:18; paddingY:0;');
    $write->printRow();

    $write->easyCell('', 'colspan:48; paddingY:0;');
    $write->easyCell($signatory_position, 'colspan:34; paddingY:0; align:C; ');
    $write->easyCell('', 'colspan:18; paddingY:0;');
    $write->printRow();
    ...
    $q22 = mysqli_query($conn,"SELECT * FROM `geo_bpls_signatory` where target_file = 'business_retirement_bplo'");
    $r22 = mysqli_fetch_assoc($q22);
    $signatory_name = $r22["signatory_name"];
    $signatory_position = $r22["signatory_position"];
    ...
    $q22 = mysqli_query($conn,"SELECT * FROM `geo_bpls_signatory` where target_file = 'business_retirement_clerk'");
    $r22 = mysqli_fetch_assoc($q22);
    $signatory_name = $r22["signatory_name"];
    $signatory_position = $r22["signatory_position"];

    $write->easyCell('', 'colspan:10; paddingY:0;');
    $write->easyCell($signatory_name, 'colspan:90; paddingY:0; font-style:B; font-size:12;');
    $write->printRow();

    $write->easyCell('', 'colspan:14; paddingY:0;');
    $write->easyCell($signatory_position.'  ', 'colspan:86; paddingY:0;');
    $write->printRow();
    ...
    $q22 = mysqli_query($conn,"SELECT * FROM `geo_bpls_signatory` where target_file = 'business_retirement_mo'");
    $r22 = mysqli_fetch_assoc($q22);
    $signatory_name = $r22["signatory_name"];
    $signatory_position = $r22["signatory_position"];
    // $e_signatory = $r22["e_signatory"];
    
    $write->easyCell($signatory_name, 'colspan:100; paddingY:0; font-style:B; font-size:12; align:C;');
    $write->printRow();

    $write->easyCell($signatory_position, 'colspan:100; paddingY:0; align:C;');
    $write->printRow();
