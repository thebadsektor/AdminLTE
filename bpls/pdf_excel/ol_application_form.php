...
if ($f_r["p_count"] > 0) {
    ...
    while ($r = mysqli_fetch_assoc($q)) {
        ...
        $q0 = mysqli_query($conn, "SELECT assessment_tax_due, natureOfCollection_tbl.name as sub_account_title from geo_bpls_assessment inner join geo_bpls_tfo_nature on geo_bpls_tfo_nature.tfo_nature_id =  geo_bpls_assessment.tfo_nature_id inner join natureOfCollection_tbl on natureOfCollection_tbl.id =  geo_bpls_tfo_nature.sub_account_no
                        where md5(geo_bpls_assessment.permit_id) = '$id' and geo_bpls_tfo_nature.nature_id = '$nature_id' and geo_bpls_tfo_nature.sub_account_no = '$gross_sub_acc' ");

        $r0 = mysqli_fetch_assoc($q0);

        $write->easyCell('', 'colspan:10; border:LB;');
        $write->easyCell($r0["sub_account_title"], 'colspan:240; border:TRB; align:L; ');
        ...
        $q2 = mysqli_query($conn, "SELECT natureOfCollection_tbl.id as sub_account_no, assessment_tax_due,natureOfCollection_tbl.name as sub_account_title from geo_bpls_assessment  inner join geo_bpls_tfo_nature on geo_bpls_tfo_nature.tfo_nature_id =  geo_bpls_assessment.tfo_nature_id inner join natureOfCollection_tbl on natureOfCollection_tbl.id =  geo_bpls_tfo_nature.sub_account_no
         where md5(geo_bpls_assessment.permit_id) = '$id' and geo_bpls_tfo_nature.nature_id = '$nature_id' and geo_bpls_tfo_nature.sub_account_no != '$gross_sub_acc' ");
        ...
        while ($r2 = mysqli_fetch_assoc($q2)) {
            ...
            if($r2["assessment_tax_due"] != 0 || $r2["assessment_tax_due"] != 0.00) {
                if($r2["sub_account_no"] != "$gross_sub_acc") {
    ...
    $write->easyCell('FIRE SAFETY INSPECTION FEE (15%)', 'colspan:250; border:1; align:R; font-style:B;');
    $write->easyCell(number_format($total_fee_reg * 0.15, 2), 'colspan:120; border:1; align:R; font-style:B;');
    $write->easyCell('', 'colspan:130; border:1; align:R;');
    $write->easyCell('', 'colspan:100; border:1; align:R;');
    $write->printRow();
