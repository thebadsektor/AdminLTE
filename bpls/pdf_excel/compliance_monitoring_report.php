...
if($qtr =="1q"){
  $qtr_str = "1st QUARTER";
  $qtr_month = "(January, Febuary, March)";
  $from = date("Y")."-01-01";
  $to = date("Y")."-03-31";
}elseif($qtr =="2q"){
  $qtr_str = "2nd QUARTER";
  $qtr_month = "(April, May, June)";
  $from = date("Y")."-04-01";
  $to = date("Y")."-06-31";

}elseif($qtr =="3q"){
  $qtr_str = "3rd QUARTER";
  $qtr_month = "(July, August, September)";
  $from = date("Y")."-07-01";
  $to = date("Y")."-09-31";
}elseif($qtr =="4q"){
  $qtr_str = "4th QUARTER";
  $qtr_month = "(October, November, December)";
  $from = date("Y")."-10-01";
  $to = date("Y")."-12-31";
}
...
$write->easyCell('IV. Data on business population and revenue fron business','colspan:70; paddingY:0; border:LB; font-style:BI;');
        ...
        if($qtr != "1q"){
         ...
         if($qtr == "2q"){
                ...
                $query22 = mysqli_query($conn, $qt2);
                $qqq22 = mysqli_fetch_assoc($query22);
                $com_qtr_month = "(Jan.,Feb.,Mar.,Apr.,May.,Jun)";
        }elseif ($qtr == "4q") {
                $from = date("Y")."-01-01";
                $to = date("Y")."-12-31";
                $qt1 = "SELECT count(permit_id) AS t_count FROM `geo_bpls_business_permit` WHERE permit_released_date BETWEEN '$from' and '$to' and geo_bpls_business_permit.step_code = 'RELEA'";
                ...
                $query22 = mysqli_query($conn, $qt2);
                $qqq22 = mysqli_fetch_assoc($query22);
                $com_qtr_month = "(Jan.,Feb.,Mar.,Apr.,May.,Jun.,Jul.,Aug.,Sep)";
        }

            $comulative_str .= "COMULATIVE ".$com_qtr_month;
...
$write->easyCell(date("Y").' '.$qtr_str.' ('.$q11["t_count"].')','colspan:20; paddingY:0; font-size:9; font-style:B; border:B;');
