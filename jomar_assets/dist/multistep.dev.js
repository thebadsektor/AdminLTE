...
DOMstrings.stepsBar.addEventListener('click', function (e) {
  ...
  $(function () {
    if ($(".step_btn" + activeStep).attr("step_status") == "lock_btn") {
      myalert_warning_af("Follow step by step process!!");
    } else {
      //set all steps before clicked (and clicked too) to active
...
DOMstrings.stepsForm.addEventListener('click', function (e) {
  ...
  if (!(eventTarget.classList.contains("".concat(DOMstrings.stepPrevBtnClass)) || eventTarget.classList.contains("".concat(DOMstrings.stepNextBtnClass)))) {
    ...
    var activePanelNum = Array.from(DOMstrings.stepFormPanels).indexOf(activePanel); // check if triger prev
    ...
    if (eventTarget.classList.contains("".concat(DOMstrings.stepPrevBtnClass))) {
      ...
      if (activePanelNum == 0) {
        ...
        $(function () {
          ...
          if (error_count > 0) {
            myalert_warning_af("Please Check your inputs");
          } else {
            // Save Info
            $(document).ready(function () {
              ...
              $(".w35343_btn").click(function () {
                ...
                $.ajax({
                  ...
                  success: function success(result) {
                    if (btn_name == "save") {
                      myalert_success_af("Application form submited!");
                    } else {
                      myalert_success_af("Application form updated!");
                    }
                ...
                setActiveStep(activePanelNum);
                setActivePanel(activePanelNum); //   unlock top clickable badge
        ...
        var a = 0; //   check  assess  using btn existence
        ...
        if (a == 1) {
          myalert_danger_af("Please finish your assessment!");
        } else {
          // no error traces
          //set active step and active panel onclick
          if (eventTarget.classList.contains("".concat(DOMstrings.stepPrevBtnClass))) {
        ...
        error_counter_r = 0; //   radio checking 
        //   if step is 0 or first
        ...
        if (error_counter_r == 1) {
          ...
          //   error message
          myalert_warning_af("Only approved application can proceed to payment");
        }
